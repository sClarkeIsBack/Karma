...
##########################################
def catchup():
    loginurl   = "http://716iptv.ddns.net:25461/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
        ...
        print e.getcode()
        xbmcgui.Dialog().ok("[COLOR white]Expired Account[/COLOR]",'[COLOR white]You cannot use this service with an expired account[/COLOR]',' ','[COLOR white]Please check your account information[/COLOR]')
        sys.exit(1)
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok(userinfo.addon_name, 'iVue Integration Complete. You may need to enable the iVue addon in your settings.')
		elif dialog==1:
			pvrsetup()
...
def tvguidesetup():
		...
		if dialog:
			...
			if dialog==0:
				...
				pvrsetup()
				ivueint()
				xbmcgui.Dialog().ok(userinfo.addon_name, 'PVR & iVue Integration Complete. You may need to enable the iVue addon in your settings.')
